...
def append_base(linuxrc, kernel_dir_opt, arch, master_config, libdir, temp, oldconfig, menuconfig, allyesconfig, mrproper, verbose):
    ...
    # if provided -m 660 /init will complain about tty1 not there
    utils.sprocessor('mknod        tty1    c 4 1', verbose)
...
def append_busybox(arch, bbconf, master_config, libdir, temp, oldconfig, menuconfig, allyesconfig, mrproper, busyboxprogs, nocache, verbose): #quiet):
	"""
...
#	@arg quiet		string
	...
	print green(' * ') + turquoise('initramfs.append_busybox ') + master_config['bb_ver'] + ' ' + busyboxprogs,
...
#					quiet)
	...
	#~ for i in ['[', 'ash', 'sh', 'mount', 'uname', 'echo', 'cut', 'cat']:
